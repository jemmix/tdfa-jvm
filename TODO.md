...
- [ ] **`(a*)(a*)` crashes at compile time** — `ArrayIndexOutOfBoundsException` in `Optimize.findFinalRegBase` (Optimize.java:71), on both ASM and VM, in both PERL and POSIX modes. Affects `(a+)(a*)`, `(a*)(a?)`, `(.*)(.*)`, `(a*)(a*)(a*)` — any pattern where a capture group can match empty adjacent to another greedy group. Root cause: the BT22 §6.4 fixed-tags pass drops a tag (e.g. "dropped 1/4" for `(a*)(a*)`), but `findFinalRegBase` still assumes a full `tagCount`-sized final block at the top of the allocation vector `V`; after the first regopt iteration shrinks `V` below `tagCount`, `oldFinalBase = V.length - tagCount` goes negative. Disappears with `-Dtdfa.noregopt=true`. java.util.regex returns `g1=[0,3] g2=[3,3]`. Tests: `CaptureGroupAllocationTest` (gated `tdfa.pending`).
...
- [ ] **`(?iu)` char-class ranges don't include Unicode fold equivalents** — `Parser.parseClass` folds class ranges ASCII-only (A-Z/a-z). `(?iu)[r-t]` should match `ſ` (U+017F, fold-equivalent of `s` in range); we return null, java.util.regex matches. Fix: use `CaseFoldTable.foldRanges` for range expansion when `unicodeShorthand && caseInsensitive`. Tests: `CaseInsensitiveTest.rangeClassShouldIncludeFoldEquivalent` (gated `tdfa.pending`).
- [ ] **`(?iu)` negated classes don't exclude Unicode fold equivalents** — `(?iu)[^s]` wrongly matches `ſ`; java.util.regex returns null. Fix is the same range-fold expansion as above (fold members are added to the positive set before `CharClass` negation is applied, so they're excluded automatically). Tests: `CaseInsensitiveTest.negatedClassShouldExcludeFoldEquivalent` (gated `tdfa.pending`).
- [ ] **`JdkUnicodeDataProvider.foldTableFor` incomplete** — `foldTableFor("Ll")` returns null (only the Lu direction is implemented; Greek/Cyrillic/L partial), so `(?i)\p{Ll}` on `A` fails with the default JDK provider. java.util.regex matches. The re2j parity suite misses this because it uses `Re2jUnicodeProvider`, which has complete fold tables. Tests: `CaseInsensitiveTest.unicodePropertyFold` (gated `tdfa.pending`).
