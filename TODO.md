...
- [x] Revisit internals access: replace the strategy-trace hook (`-Dtdfa.trace.strategy`, `TdfaRunner.traceSnapshot`) with first-class observer/event API — RESOLVED AS OUT OF SCOPE (2026-09-19): Phase C (the API-surface review) closed in r9 without adopting an observer/event API; the trace hook remains a conformance instrument, and pipeline exposure (String regex → AST → TNFA → TDFA) is not part of the 1.0 surface.
...
- [x] CI performance regression tracking — the harness landed (P7: `RegressionBench`/
      `QuickBench`, `scripts/bench-regression.sh [--jmh] [--capture]`,
      `bench-compare.py`, per-machine baselines — quick threshold 15 %, JMH 10 %).
      RESOLVED AS WAIVED (2026-09-19): perf regression stays a manual/local gate
      against per-machine baselines; CI wiring waived — shared-runner timing noise
      makes a CI threshold meaningless (no threshold decision can fix that).
      Reopen only with self-hosted or otherwise timing-stable runners.
...
- [ ] Qodana — decide final disposition (review later, before freeze): keep or
      remove the root-plugin + `qodana.yaml` wiring. Declined as a one-off
      (r9 §1b); REVIEW r10 §3 flags the wiring as dead weight for a declined
      tool (pure download + config surface, run by nothing).
- [ ] JaCoCo coverage targets (line + branch)
- [ ] JavaDoc for all public API surface
- [ ] API stability guarantees (signatures locked at 1.0)
- [x] ~~Multi-JDK CI matrix~~ — SUPERSEDED SHAPE; CI live since 2026-09-04
...
- [x] Memory leak testing (generated class GC under load) — RESOLVED
      (2026-09-19): the 10 k-pattern load/unload probe is recorded in
      BENCHMARKS §5 — generated-class metaspace is proportional to live
      patterns and unloads with the pattern (no retention under churn).
