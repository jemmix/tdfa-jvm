...
- [x] **O(n²) unanchored `find()`** — fixed via multi-state parallel simulation in `TdfaRunner.multiStateAnyMatch`: a single forward pass tracks the set of all DFA states reachable from any start position (O(n × |states|)), replacing the outer-loop restart. The VM backend uses it directly for boolean `find()` and as a no-match pre-check for extract; the ASM backend delegates `find()` to the VM runner and uses the runner's `anyMatch` as a pre-check before `runExtract`. 200 K-char no-match haystack: ~14 ms (was >30 s).
...
- [x] Tracer-bullet parity test against rebar scenarios — `:tests:parity:rebar:RebarScenarioParityTest`. With the loader now spec-compliant (per-line alternate/pattern, prepend/append, literal, array→alternation, `re2` count selection) and the engine running in its real default mode (ASM + PERL), and the **scope now restricted to scenarios rebar actually tests against Java** (`java/hotspot` in engines list — see `docs/PARITY-PLAN.md`), with size caps lifted to cover all in-scope haystacks and `compile`/`grep-captures` models implemented, **72 of 76 runnable** scenarios pass; 4 surface known engine bugs (see "Correctness" below); 283 skip on scope/size/model filters (the `dot-matches-byte` and `unicode-spans` architectural divergences are patched in the vendored corpus — see `vendor/patches/rebar/`).
- [x] Expand rebar parity — bumped `MAX_HAYSTACK_BYTES` (200 KB → 16 MB) / `MAX_REGEX_LEN` (2 KB → 32 KB), raised run/compile timeouts to rebar's own ceilings, added `grep-captures` and `compile` models (PARITY-PLAN Phases 2.3/3/4.1/4.2). Surfaced 4 new engine bugs (see "Correctness" below).
- [ ] Remaining rebar parity — Phase 5 engine bug fixes (see "Correctness").
