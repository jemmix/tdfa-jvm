...
- **Cost, stated up front**: compile is ~52 µs (VM) / ~301 µs (ASM) per pattern vs ~4 µs for `java.util.regex` — eager AOT determinization is the price of the linear-time guarantee. Patterns whose TDFA would exceed the determinization budget (re2c's own design: 100 K states / 50 M kernel-total, overridable via `-Dtdfa.max.states` / `-Dtdfa.max.kernels`) fail compilation with a clean "pattern too large" error rather than exhausting time and memory.
