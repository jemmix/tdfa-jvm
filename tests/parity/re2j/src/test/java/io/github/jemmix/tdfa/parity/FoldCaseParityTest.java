...
class FoldCaseParityTest {
    ...
    void historicCyrillicAgreesWithOracleUniverse(RegexEngineFactory factory) {
        for (String p : new String[]{
            ...
            if (releasedOracle() && containsHistoricLetter(p)) continue;
    ...
    void defaultUniverseFoldsModernOrbits() {
        ...
        assertThat(tdfaFindDefaultUniverse("(?i)\u0442", "\u1C85"))
                .as("default universe (?i)т → ꚅ (modern orbit)").isNotNull();
        assertThat(tdfaFindDefaultUniverse("(?i)\u1C85", "\u0442"))
                .as("default universe (?i)ꚅ → т (modern orbit, both directions)").isNotNull();
        assertThat(tdfaFindDefaultUniverse("(?i)\u1C80", "\u0432"))
                .as("default universe (?i)Ꚁ → в (modern orbit)").isNotNull();
        // ... and self-match, as every fold universe must.
        assertThat(tdfaFindDefaultUniverse("(?i)\u1C80", "\u1C80"))
                .as("default universe (?i)Ꚁ → Ꚁ").isNotNull();
    }
    ...
    void oracleDoesNotFoldModernHistoricCyrillic(RegexEngineFactory factory) {
        ...
        assertThat(re2jFind("(?i)\u0442", "\u1C85"))
                .as("oracle (?i)т → ꚅ (post-6.0 orbits not folded)").isNull();
        // under the bridge tdfa agrees with the oracle on that cell ...
        assertThat(tdfaFind("(?i)\u0442", "\u1C85", factory))
                .as("bridge universe tdfa (?i)т → ꚅ agrees with oracle").isNull();
