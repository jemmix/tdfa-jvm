...
class FoldCaseParityTest {
    ...
    void oracleDoesNotFoldModernHistoricCyrillic(RegexEngineFactory factory) {
        ...
        assertThat(re2jFind("(?i)\u0442", "\u1C85"))
                .as("oracle (?i)т → ꚅ (post-6.0 orbits not folded)")
                .isNull();
        // under the bridge tdfa agrees with the oracle on that cell ...
        assertThat(tdfaFind("(?i)\u0442", "\u1C85", factory))
                .as("bridge universe tdfa (?i)т → ꚅ agrees with oracle")
                .isNull();
