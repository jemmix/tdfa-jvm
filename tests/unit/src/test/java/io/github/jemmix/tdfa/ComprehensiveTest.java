...
class ComprehensiveTest {
    ...
    void testVm(Case c) {
        ...
        assertThat(result).as("VM: %s (pattern=%s, input=%s)", c.label(), c.pattern(), c.input()).isEqualTo(c.match());
    ...
    void testAsm(Case c) {
        ...
        Matcher mm = r.matcher(c.input());
        boolean result = c.useFind() ? mm.find() : mm.matches();
        assertThat(result).as("ASM: %s (pattern=%s, input=%s)", c.label(), c.pattern(), c.input()).isEqualTo(c.match());
        ...
        if (c.match() && c.groups() != null && r.groupCount() > 0) {
            ...
            int[] expected = c.groups();
            for (int g = 0; g <= r.groupCount(); g++) {
                assertThat(m.start(g)).as("ASM group %d start: %s", g, c.label()).isEqualTo(expected[g * 2]);
