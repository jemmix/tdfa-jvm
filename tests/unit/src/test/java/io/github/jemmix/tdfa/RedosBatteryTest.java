...
class RedosBatteryTest {
    ...
    private static final long REJECT_BOUND_MS =
            60_000 * CI_MULT; // cap-crossing abort: measured 6–19 s on the shapes below
    ...
    static Stream<Case> evilPatterns() {
        return Stream.of(
                ...
                new Case(
                        "two-site-wide-repeat",
                        "[\\s\\S]{0,100}x[\\s\\S]{0,100}",
                        n -> repeat('a', PROBE_N / 2) + "x" + repeat('b', PROBE_N / 2),
                        true));
    }
    ...
    void linearTimeUnderAttack(Case c, RegexEngineFactory factory) {
        ...
        try {
            ...
            assertThat(ms)
                    .as("%s: budget rejection must be prompt (%d ms)", c, ms)
        ...
        assertThat(compileMs)
                .as("%s: compile must stay bounded (%d ms)", c, compileMs)
        ...
        assertThat(matchMs)
                .as("%s: adversarial match must stay bounded (%d ms)", c, matchMs)
        ...
        if (c.scalingProbe()) {
            ...
            assertThat(ratio)
                    .as(
                            "%s: doubling %d→%d chars must not blow up (t=%d→%d ms, ratio=%.1f)",
    ...
    void stateBombsRejectCleanly(String name, String pattern, RegexEngineFactory factory) {
        ...
        try {
            ...
            assertThat(e.getMessage())
                    .as(name + ": rejection must be the documented too-large error")
