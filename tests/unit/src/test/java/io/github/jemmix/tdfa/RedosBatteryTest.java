...
class RedosBatteryTest {
    ...
    static Stream<Case> evilPatterns() {
        return Stream.of(new Case("nested-plus", "(a+)+b", n -> repeat('a', 40) + "X", false), new Case("nested-alt",
                        "(a|aa)+b", n -> repeat('a', 40) + "X",
                        false), new Case("optional-star-alt", "(a|a?)+b", n -> repeat('a', 40) + "X", false), new Case(
                                        "star-of-star", "(a*)*b", n -> repeat('a', 40) + "X",
                                        false), new Case("plus-of-star", "(a+)*b", n -> repeat('a', 40) + "X",
                                                        false), new Case("triple-nested", "((a+)*)+b",
                                                                        n -> repeat('a', 40) + "X",
                                                                        false), new Case("double-x", "(x+x+)+y",
                                                                                        n -> repeat('x', 60) + "z",
                                                                                        true), new Case("alt-group-repeat",
                                                                                                        "(a|b|ab)*c",
                                                                                                        n -> "ab".repeat(30) + "d",
                                                                                                        false), new Case(
                                                                                                                        "ab-star-nested",
                                                                                                                                        ...
                                                                                                                                        false), new Case(
                                                                                                                                                        ...
                                                                                                                                                        false), new Case(
                                                                                                                                                                        ...
                                                                                                                                                                        false), new Case(
                                                                                                                                                                                        "unicode-letters",
                                                                                                                                                                                        "(\\p{L}+)+\\d",
                                                                                                                                                                                        n -> "αβγδεζηθ".repeat(10) + "!",
                                                                                                                                                                                        false), new Case(
                                                                                                                                                                                                        "dot-star-groups",
                                                                                                                                                                                                        "(.*)(.*)(.*)(.*)a",
                                                                                                                                                                                                        n -> repeat('b', 100) + "c",
                                                                                                                                                                                                        true), new Case("overlapping-alt",
                                                                                                                                                                                                                        "(a|b)*c",
                                                                                                                                                                                                                        n -> repeat('a', 2000) + "d",
                                                                                                                                                                                                                        true), new Case("group-optional-loop",
                                                                                                                                                                                                                                        "^([a-zA-Z0-9]+)*$",
                                                                                                                                                                                                                                        n -> repeat('a', 60) + "-",
                                                                                                                                                                                                                                        false), new Case(
                                                                                                                                                                                                                                                        "suffix-fail-loop",
                         ...
                         new Case("two-site-wide-repeat", "[\\s\\S]{0,100}x[\\s\\S]{0,100}",
                                         n -> repeat('a', PROBE_N / 2) + "x" + repeat('b', PROBE_N / 2), true));
    }

    static Stream<Arguments> casesByEngine() {
        List<Arguments> out = new ArrayList<>();
        for (RegexEngineFactory factory : engineFactories().toList()) {
    ...
    static Stream<RegexEngineFactory> engineFactories() {
        return Stream.<RegexEngineFactory>of(null, TdfaRunner::new);
    }

    @ParameterizedTest(name = "{0}")
    @MethodSource("casesByEngine")
    void linearTimeUnderAttack(Case c, RegexEngineFactory factory) {
        ...
        assertThat(compileMs).as("%s: compile must stay bounded (%d ms)", c, compileMs).isLessThan(COMPILE_BOUND_MS);
        ...
        assertThat(matchMs).as("%s: adversarial match must stay bounded (%d ms)", c, matchMs).isLessThan(MATCH_BOUND_MS);
        ...
        if (c.scalingProbe()) {
            ...
            assertThat(ratio).as("%s: doubling %d→%d chars must not blow up (t=%d→%d ms, ratio=%.1f)", c, PROBE_N, PROBE_N * 2, tN, t2N, ratio).isLessThan(SCALE_RATIO_MAX);
        }
    }
    ...
    static Stream<Arguments> bombPatterns() {
        ...
        for (RegexEngineFactory factory : engineFactories().toList()) {
            ...
            out.add(Arguments.of("state-cap-context", "[A-Za-z]{10}\\s+[\\s\\S]{0,100}Result[\\s\\S]{0,100}\\s+[A-Za-z]{10}", factory));
            // kernel-total-cap rejection (measured ~6–10 s at 62 334 states / 50 M kernels; needs ≥2 GB heap).
            out.add(Arguments.of("kernel-cap-two-site-word", "\\w{0,400}y\\w{0,400}", factory));
        }
        return out.stream();
    }
    ...
    void stateBombsRejectCleanly(String name, String pattern, RegexEngineFactory factory) {
        long t0 = System.nanoTime();
        try {
            ...
            assertThat(e.getMessage()).as(name + ": rejection must be the documented too-large error").containsAnyOf("too large", "too many");
