...
class RedosBatteryTest {
    ...
    static Stream<Case> evilPatterns() {
        return Stream.of(new Case("nested-plus", "(a+)+b", n -> repeat('a', 40) + "X", false),
    ...
    void linearTimeUnderAttack(Case c, RegexEngineFactory factory) {
        ...
        if (c.scalingProbe()) {
            ...
            assertThat(ratio).as("%s: doubling %d→%d chars must not blow up (t=%d→%d ms, ratio=%.1f)", c, PROBE_N,
                PROBE_N * 2, tN, t2N, ratio).isLessThan(SCALE_RATIO_MAX);
        }
    }
