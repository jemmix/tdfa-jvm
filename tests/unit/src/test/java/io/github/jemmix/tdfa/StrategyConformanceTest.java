...
class StrategyConformanceTest {
    ...
    private static final List<String[]> SHAPES = List.of(new String[][]{{"Twain", "The adventures of Tom Sawyer and Huckleberry Finn, by Mark Twain."}, {"zzqqxv", "The adventures of Tom Sawyer and Huckleberry Finn, by Mark."}, {"(?i)sherlock", "Mr Sherlock Holmes, the consulting detective, walked in."}, {"\\bword\\b", "a short sentence with word inside the text here"}, {"\\p{L}{2,}", "Привет мир, вот тестовое предложение короткое"}, {"[а-яА-ЯёЁ]{4,}", "Привет мир, вот тестовое предложение короткое"}, {"\"[^\"]{5,20}\"", "He said \"hello world\" today and left quite quietly"}, {"(\\d+)\\.(\\d+)\\.(\\d+)\\.(\\d+)", "connecting from ip=192.168.1.77 port 443 ok"}, {"(a|b)*c", "aabbaabbc"}, {"\\w+@\\w+\\.(com|org|net)", "no addresses anywhere in this particular line at all"}, {"[a-z]+qrst", "the quick brown fox jumps over the lazy dog qrstxx"}, {"(?m)^line", "first\nline two\nline three\nline four\nline five"},
    ...
    private static final int[] LENGTHS = {1, 2, 3, 15, 40, 63, 64, 65, 100, 127, 128, 129, 255, 256, 257, 300, 2047, 2048, 2049, 4100};
    ...
    private static int compare(RegexEngine vm, RegexEngine asm, String in, String ctx) {
        ...
        assertThat(asm.matchWhole(cs, new MatchScratch()) == null).as("%s: matchWhole(CharSequence) nullity", ctx).isEqualTo(vm.matchWhole(cs, new MatchScratch()) == null);
        return 9;
    }
