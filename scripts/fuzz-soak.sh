...
#
#   scripts/fuzz-soak.sh [iterations=480] [minutes-per-iter=1] [out=build/fuzz] [seed-base=random]
#
...
# and heap until the run degrades or OOMs (the first soak: 74% of wall time
# waiting on oracle hangs, throughput 11k -> 7.5k cases/min and falling). One
# JVM per iteration bounds every leak to the chunk length; ~3-5 s of
# gradle+JIT overhead per restart is the price.
#
...
mkdir -p "$OUT"
echo "$(date '+%F %T') soak start: iters=$ITERS mins=$MINS base-seed=$BASE out=$OUT" >> "$LOG"
for i in $(seq 1 "$ITERS"); do
    ...
    if ./gradlew -q :tests:parity:re2j:fuzz -Pfuzz.minutes="$MINS" -Pfuzz.seed="$seed" \
            -Pfuzz.out="$OUT" $append >> "$LOG" 2>&1; then
